...
```bash
npm install
npm run dev          # http://localhost:5173/product-experience-center/
...
배포 URL 예시: `https://<owner>.github.io/product-experience-center/`

`vite.config.ts` 의 `base: '/product-experience-center/'` 가 서브패스 정적 자산을 처리합니다. SPA 라우팅은 HashRouter 를 사용하므로 GH Pages 의 404 fallback 우회 설정이 필요하지 않습니다.
